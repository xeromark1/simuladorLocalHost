                    ...
                    var diccionary = {};
                        ...
                        for(let index = 0 ; index < sessionStorage.length ; index++){   //recorremos todas las cookies si es que tenemos
                            if(index+1 != i){ // es necesario no incluir la materia que se esta cambiando
                                let m = index+1;
                                let coockies = JSON.parse(sessionStorage.getItem("codigo"+m));

                                for(let coockie of coockies){   //dentro de esos elemenos, hay distintos eventos con distintas horas y dias
                                    var bloqueHoraCookie = getHoras(coockie.hora);
                                    var bloqueDiasCookie = getDias(coockie.dia);
...
//arreglamos la combobox
                        ...
                        if(aux4.indexOf(dato.codigo) == -1  && SelectionComboBoxSeccion != "" && SelectionComboBoxSeccion != null){
                            ...
                            if(choqueDeDias == true){
                                diccionary[dato.codigo] = '<option value="' + dato.codigo + '" disabled>' + dato.codigo+ ' | '  /*+ dato.dia + " " + dato.hora  + ' | ' */ + dato.nombre +'</option>';
